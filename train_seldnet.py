...
def main(argv):
    ...
    if params['mode'] == 'dev':
        if '2020' in params['dataset_dir']:
            ...
            exit()
    for split_cnt, split in enumerate(train_splits):
        print('\n\n---------------------------------------------------------------------------------------------------')
        ...
        unique_name = '{}_{}_{}_split{}_{}_{}'.format(
            task_id, job_id, params['mode'], split, loc_output, loc_feat
        )
        ...
        df = pd.DataFrame(columns=['time', 'epoch', 'train loss', 'ER', 'F', 'LE', 'LR', 'SELD', 'best_epoch'])
        df.to_csv('csvlog/{}_loss.csv'.format(unique_name), index=False)
        for epoch_cnt in range(nb_epoch):
            ...
            p_time = datetime.now()
            p_epoch = epoch_cnt
            p_trainloss = train_loss
            p_er = val_ER
            p_f = val_F
            p_le = val_LE
            p_lr = val_LR
            p_seld = val_seld_scr
            p_bestepoch = best_val_epoch
            list = [p_time, p_epoch, p_trainloss, p_er, p_f, p_le, p_lr, p_seld, p_bestepoch]
            data = pd.DataFrame([list])
            data.to_csv('csvlog/{}_loss.csv'.format(unique_name), mode='a', header=False, index=False)
            if epoch_cnt > (best_val_epoch+params['patience']):
                break
        # ---------------------------------------------------------------------
