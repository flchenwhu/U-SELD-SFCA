...
def main(argv):
    ...
    data_gen_test = cls_data_generator.DataGenerator(
        params=params, split=4, shuffle=True, is_eval=True if params['mode']=='eval' else False
    )
    ...
    with torch.no_grad():
        ...
        for data, target in data_gen_test.generate():
            ...
            if use_activity_detector:
                activity = (torch.sigmoid(output).cpu().detach().numpy() >0.5)
            ...
            if dump_figures:
                ...
                plot.savefig(fig_name, dpi=150)
                plot.close()
                file_cnt += 1
            else:
                plot.savefig('Compare/ablation.png', bbox_inches='tight', pad_inches=0.0, dpi=400)
                plot.show()
            if file_cnt>2:
                break
